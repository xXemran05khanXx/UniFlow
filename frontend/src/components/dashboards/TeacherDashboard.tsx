...
function toSubjectProgress(sessions: Session[], now: Date): SubjectProgress[] {
  ...
  sessions.forEach(s => {
    ...
    if (!map.has(key)) map.set(key, {
      id: s.courseId || key, name: s.courseName || 'N/A', code: s.courseCode || 'N/A',
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {cls.dynamicStatus && cls.dynamicStatus !== 'Regular' && (
                        <span className={`px-2 py-1 text-xs font-bold rounded-full ${cls.dynamicStatus === 'Swapped In' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                          }`}>
                          {cls.dynamicStatus}
                        </span>
                      )}
